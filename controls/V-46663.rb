...
# impacts
title 'V-46663 - Java permissions must be disallowed (Locked Down Restricted Sites zone).'
control 'V-46663' do
  impact 0.5
  ...
  tag 'stig', 'V-46663'
  tag severity: 'medium'
  tag checkid: 'C-49821r2_chk'
  tag fixid: 'F-50431r1_fix'
  tag version: 'DTBI450-IE11'
  tag ruleid: 'SV-59527r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Locked-Down Restricted Sites Zone -> Java permissions to Enabled, and select Disable Java from the drop-down box.  '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Locked-Down Restricted Sites Zone -> Java permissions must be Enabled, and Disable Java selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following keys: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Lockdown_Zones\4 Criteria: If the value "1C00" is REG_DWORD = 0, this is not a finding.'

# START_DESCRIBE V-46663
