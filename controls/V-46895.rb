...
# impacts
title 'V-46895 - Cross-Site Scripting Filter property must be enforced (Restricted Sites zone).'
control 'V-46895' do
  impact 0.5
  ...
  tag 'stig', 'V-46895'
  tag severity: 'medium'
  tag checkid: 'C-49951r2_chk'
  tag fixid: 'F-50631r1_fix'
  tag version: 'DTBI890-IE11'
  tag ruleid: 'SV-59761r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Turn on Cross-Site Scripting Filter to Enabled, and select Enable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Turn on Cross-Site Scripting Filter must be Enabled, and Enable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\4 Criteria: If the value "1409" is REG_DWORD = 0, this is not a finding.'

# START_DESCRIBE V-46895
