...
# impacts
title 'V-46921 - .NET Framework-reliant components signed with Authenticode must be disallowed to run (Internet zone).'
control 'V-46921' do
  impact 0.5
  ...
  tag 'stig', 'V-46921'
  tag severity: 'medium'
  tag checkid: 'C-49959r2_chk'
  tag fixid: 'F-50659r1_fix'
  tag version: 'DTBI930-IE11'
  tag ruleid: 'SV-59787r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone Run .NET Framework-reliant components signed with Authenticode to Enabled, and select Disable from the drop-down box.  '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone Run .NET Framework-reliant components signed with Authenticode must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3 Criteria: If the value "2001" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46921
