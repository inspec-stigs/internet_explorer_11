...
# impacts
title 'V-46509 - Accessing data sources across domains must be disallowed (Internet zone).'
control 'V-46509' do
  impact 0.5
  ...
  tag 'stig', 'V-46509'
  tag severity: 'medium'
  tag checkid: 'C-49699r2_chk'
  tag fixid: 'F-50299r1_fix'
  tag version: 'DTBI032-IE11'
  tag ruleid: 'SV-59373r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Access data sources across domains to Enabled, and select Disable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Access data sources across domains must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3 Criteria: If the value "1406" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46509
