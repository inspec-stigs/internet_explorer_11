...
# impacts
title 'V-46643 - Automatic prompting for file downloads must be disallowed (Internet zone).'
control 'V-46643' do
  impact 0.5
  ...
  tag 'stig', 'V-46643'
  tag severity: 'medium'
  tag checkid: 'C-49809r2_chk'
  tag fixid: 'F-50413r1_fix'
  tag version: 'DTBI415-IE11'
  tag ruleid: 'SV-59507r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Automatic prompting for file downloads to Enabled, and select Disable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Automatic prompting for file downloads must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3 Criteria: If the value "2200" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46643
