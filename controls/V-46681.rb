...
# impacts
title 'V-46681 - Protected Mode must be enforced (Internet zone).'
control 'V-46681' do
  impact 0.5
  ...
  tag 'stig', 'V-46681'
  tag severity: 'medium'
  tag checkid: 'C-49835r2_chk'
  tag fixid: 'F-50447r1_fix'
  tag version: 'DTBI485-IE11'
  tag ruleid: 'SV-59545r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Turn on Protected Mode to Enabled, and select Enable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Turn on Protected Mode must be Enabled, and Enable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3 Criteria: If the value "2500" is REG_DWORD = 0, this is not a finding.'

# START_DESCRIBE V-46681
