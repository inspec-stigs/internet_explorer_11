...
# impacts
title 'V-46501 - The Initialize and script ActiveX controls not marked as safe property must be disallowed (Internet zone).'
control 'V-46501' do
  impact 0.5
  ...
  tag 'stig', 'V-46501'
  tag severity: 'medium'
  tag checkid: 'C-49693r2_chk'
  tag fixid: 'F-50291r1_fix'
  tag version: 'DTBI024-IE11'
  tag ruleid: 'SV-59365r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Initialize and script ActiveX controls not marked as safe to Enabled, and select Disable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Initialize and script ActiveX controls not marked as safe must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3 Criteria: If the value "1201" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46501
