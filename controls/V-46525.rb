...
# impacts
title 'V-46525 - Java permissions must be configured with High Safety (Intranet zone).'
control 'V-46525' do
  impact 0.5
  ...
  tag 'stig', 'V-46525'
  tag severity: 'medium'
  tag checkid: 'C-49713r2_chk'
  tag fixid: 'F-50313r1_fix'
  tag version: 'DTBI061-IE11'
  tag ruleid: 'SV-59389r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Intranet Zone -> Java permissions to Enabled, and select High Safety from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Intranet Zone -> Java permissions must be Enabled, and High Safety selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\1 Criteria: If the value "1C00" is REG_DWORD = 65536, (Decimal), this is not a finding.'

# START_DESCRIBE V-46525
