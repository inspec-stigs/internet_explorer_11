...
# impacts
title 'V-46647 - Java permissions must be disallowed (Locked Down Local Machine zone).'
control 'V-46647' do
  impact 0.5
  ...
  tag 'stig', 'V-46647'
  tag severity: 'medium'
  tag checkid: 'C-49813r2_chk'
  tag fixid: 'F-50417r1_fix'
  tag version: 'DTBI430-IE11'
  tag ruleid: 'SV-59511r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Locked-Down Local Machine Zone -> Java permissions to Enabled, and select Disable Java from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Locked-Down Local Machine Zone -> Java permissions must be Enabled, and Disable Java selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following keys: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Lockdown_Zones\0 Criteria: If the value "1C00" is REG_DWORD = 0, this is not a finding.'

# START_DESCRIBE V-46647
