...
# impacts
title 'V-46669 - XAML files must be disallowed (Restricted Sites zone).'
control 'V-46669' do
  impact 0.5
  ...
  tag 'stig', 'V-46669'
  tag severity: 'medium'
  tag checkid: 'C-49825r2_chk'
  tag fixid: 'F-50435r1_fix'
  tag version: 'DTBI460-IE11'
  tag ruleid: 'SV-59533r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Allow loading of XAML files to Enabled, and select Disable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Allow loading of XAML files must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\4 Criteria: If the value "2402" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46669
