...
# impacts
title 'V-46885 - When uploading files to a server, the local directory path must be excluded (Restricted Sites zone).'
control 'V-46885' do
  impact 0.5
  ...
  tag 'stig', 'V-46885'
  tag severity: 'medium'
  tag checkid: 'C-49945r2_chk'
  tag fixid: 'F-50623r1_fix'
  tag version: 'DTBI860-IE11'
  tag ruleid: 'SV-59751r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Include local path when user is uploading files to a server to Enabled, and select Disable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Include local path when user is uploading files to a server must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\4 Criteria: If the value "160A" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46885
