...
# impacts
title 'V-46691 - Pop-up Blocker must be enforced (Restricted Sites zone).'
control 'V-46691' do
  impact 0.5
  ...
  tag 'stig', 'V-46691'
  tag severity: 'medium'
  tag checkid: 'C-49841r2_chk'
  tag fixid: 'F-50453r1_fix'
  tag version: 'DTBI500-IE11'
  tag ruleid: 'SV-59555r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Use Pop-up Blocker to Enabled, and select Enable from the drop-down box. '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Use Pop-up Blocker must be Enabled, and Enable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\4 Criteria: If the value "1809" is REG_DWORD = 0, this is not a finding.'

# START_DESCRIBE V-46691
