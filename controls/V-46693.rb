...
# impacts
title 'V-46693 - Websites in less privileged web content zones must be prevented from navigating into the Internet zone.'
control 'V-46693' do
  impact 0.5
  ...
  tag 'stig', 'V-46693'
  tag severity: 'medium'
  tag checkid: 'C-49843r2_chk'
  tag fixid: 'F-50455r1_fix'
  tag version: 'DTBI515-IE11'
  tag ruleid: 'SV-59557r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Web sites in less privileged Web content zones can navigate into this zone to Enabled, and select Disable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Internet Zone -> Web sites in less privileged Web content zones can navigate into this zone must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\3 Criteria: If the value "2101" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46693
