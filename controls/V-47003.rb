...
# impacts
title 'V-47003 - Anti-Malware programs against ActiveX controls must be run for the Local Machine zone.'
control 'V-47003' do
  impact 0.5
  ...
  tag 'stig', 'V-47003'
  tag severity: 'medium'
  tag checkid: 'C-49985r2_chk'
  tag fixid: 'F-50721r1_fix'
  tag version: 'DTBI426-IE11'
  tag ruleid: 'SV-59869r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer-> Internet Control Panel -> Security Page -> Local Machine Zone Dont run antimalware programs against ActiveX controls to Enabled and select Disable in the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer-> Internet Control Panel -> Security Page ->Local Machine Zone Dont run antimalware programs against ActiveX controls must be Enabled and Disable selected in the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\0  Criteria: If the value "270C" is REG_DWORD = 0, this is not a finding.'

# START_DESCRIBE V-47003
