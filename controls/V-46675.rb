...
# impacts
title 'V-46675 - MIME sniffing must be disallowed (Restricted Sites zone).'
control 'V-46675' do
  impact 0.5
  ...
  tag 'stig', 'V-46675'
  tag severity: 'medium'
  tag checkid: 'C-49829r2_chk'
  tag fixid: 'F-50441r1_fix'
  tag version: 'DTBI470-IE11'
  tag ruleid: 'SV-59539r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Enable MIME Sniffing to Enabled, and select Disable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Enable MIME Sniffing must be Enabled, and Disable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\4 Criteria: If the value "2100" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46675
