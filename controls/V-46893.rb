...
# impacts
title 'V-46893 - ActiveX controls without prompt property must be used in approved domains only (Restricted Sites zone).'
control 'V-46893' do
  impact 0.5
  ...
  tag 'stig', 'V-46893'
  tag severity: 'medium'
  tag checkid: 'C-49949r2_chk'
  tag fixid: 'F-50627r1_fix'
  tag version: 'DTBI880-IE11'
  tag ruleid: 'SV-59759r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Allow only approved domains to use ActiveX controls without prompt to Enabled, and select Enable from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Restricted Sites Zone -> Allow only approved domains to use ActiveX controls without prompt must be Enabled, and Enable selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following key: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Zones\4 Criteria: If the value "120b" is REG_DWORD = 3, this is not a finding.'

# START_DESCRIBE V-46893
