...
# impacts
title 'V-46653 - Java permissions must be disallowed (Locked Down Trusted Sites zone).'
control 'V-46653' do
  impact 0.5
  ...
  tag 'stig', 'V-46653'
  tag severity: 'medium'
  tag checkid: 'C-49817r2_chk'
  tag fixid: 'F-50423r1_fix'
  tag version: 'DTBI440-IE11'
  tag ruleid: 'SV-59517r1_rule'
  tag fixtext: 'Set the policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Locked-Down Trusted Sites Zone -> Java permissions to Enabled, and select Disable Java from the drop-down box.   '
  tag checktext: 'The policy value for Computer Configuration -> Administrative Templates -> Windows Components -> Internet Explorer -> Internet Control Panel -> Security Page -> Locked-Down Trusted Sites Zone -> Java permissions must be Enabled, and Disable Java selected from the drop-down box. Procedure: Use the Windows Registry Editor to navigate to the following keys: HKLM\Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings\Lockdown_Zones\2 Criteria: If the value "1C00" is REG_DWORD = 0, this is not a finding.'

# START_DESCRIBE V-46653
